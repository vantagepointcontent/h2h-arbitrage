...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const [, indicativeValuations] = await Promise.all([
      Promise.all(marketIds.map(async (marketId) => {
        urlByMarket.set(marketId, await getMarketUrlsById(marketId));
      })),
      enrichBotPositionsWithPersistedIndicativeValuation(positions),
    ]);
    const enrichedValuations = await enrichBotPositionsWithSettlementLedger(indicativeValuations);
    const enrichedPositions = enrichedValuations.map((position) => ({
        ...position,
