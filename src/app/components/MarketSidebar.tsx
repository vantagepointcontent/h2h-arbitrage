...
/* ── Market Sidebar ── */
export interface SavedMarketsListRefreshState {
...
function MarketSidebarInner({
  ...
  listRefreshState: SavedMarketsListRefreshState;
  onGoOverview: () => void;
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    ...
                    disabled={listRefreshState.status === 'loading'}
                    aria-label="Refresh markets"
                    className="p-1.5 rounded-md hover:bg-[var(--border-subtle)] text-[var(--text-secondary)] hover:text-[var(--status-positive)] transition-colors disabled:opacity-50"
                    title="Refresh markets from the latest persisted scanner snapshot"
                  >
                    {listRefreshState.status === 'loading' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />}
                  </button>
                </div>
              </div>
