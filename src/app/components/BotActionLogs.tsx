...
const statusLabel: Record<LogStatus,string> = { passed:'Attempt completed', failed:'Rejected / failed', pending:'Placement in progress / partial' };
...
export default function BotActionLogs({ selectionMethod }: { selectionMethod?: 'roi' | 'apy' | 'hybrid' }) {
  ...
  useEffect(()=>{ const initialId=window.setTimeout(()=>void load(),0); const intervalId=autoRefresh?window.setInterval(()=>void load(),30_000):null; return()=>{window.clearTimeout(initialId);if(intervalId!=null)window.clearInterval(intervalId);}; },[autoRefresh,load]);

  return <section className="space-y-3" aria-label="BotTrader action logs">
    <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-panel)] px-3 py-2"><div className="text-sm font-semibold text-[var(--text-primary)]">Placement attempts</div><div className="text-[10px] text-[var(--text-secondary)]">Evaluation and order-placement history. An attempt is not an open position; successful two-leg placements appear under Positions &amp; P/L.</div></div>
    ...
    {loading&&<div role="status" className="py-8 text-center text-sm text-[var(--text-secondary)]">Loading placement attempts…</div>}
    <div className="space-y-2">
      {trades.map(trade=>{const open=expanded.has(trade.tradeId);return <article key={trade.tradeId} className="overflow-hidden rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-panel)]">
        <button aria-label={`${open?'Collapse':'Expand'} attempt for ${trade.marketTitle}`} onClick={()=>setExpanded(current=>{const next=new Set(current);if(open)next.delete(trade.tradeId);else next.add(trade.tradeId);return next})} className="flex min-h-14 w-full items-center gap-3 px-3 text-left">
          {open?<ChevronDown className="h-4 w-4"/>:<ChevronRight className="h-4 w-4"/>}<span className={statusClass[trade.status]}>{statusIcon[trade.status]}</span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-[var(--text-primary)]">{trade.marketTitle}</strong><span className={`text-[10px] font-semibold ${statusClass[trade.status]}`}>{statusLabel[trade.status]}</span><span className="ml-2 text-[10px] text-[var(--text-secondary)]">Attempt only — not an open position · {new Date(trade.startedAt).toLocaleString()} · {trade.steps.length} steps · {trade.trigger}</span></span>
        </button>
