              ...
              {sortedPositions.map((position) => {
                ...
                return [
                  ...
                  isExpanded && <tr key={`detail-${position.id}`}>
                    <td colSpan={11} className="bg-[var(--surface-workspace)] px-3 py-3 sm:px-10">
                      ...
                      <div className="mb-3 grid gap-2 text-xs lg:grid-cols-2" aria-label="Exact placed venue identities">
                        <div data-testid="kalshi-placed-identity" className="rounded border border-[var(--border-subtle)] bg-[var(--surface-panel)] px-3 py-2">
                          <div className="font-semibold text-[var(--text-primary)]">Kalshi placed leg</div>
                          <div className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1"><span className="text-[var(--text-secondary)]">Selected market choice</span><span className={hasCanonicalOutcomes ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}>{hasCanonicalOutcomes ? position.kalshiOutcomeLabel!.trim() : 'Unavailable'}</span><span className="text-[var(--text-secondary)]">Side</span><span className="font-semibold text-[var(--text-primary)]">{position.kalshiSide.toUpperCase()}</span><span className="text-[var(--text-secondary)]">Platform question</span><span className={position.kalshiMarketQuestion?.trim() ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}>{position.kalshiMarketQuestion?.trim() || 'Unavailable'}</span><span className="text-[var(--text-secondary)]">Ticker</span><span className={`break-all font-mono ${position.kalshiTicker ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}`}>{position.kalshiTicker || 'Unavailable'}</span></div>
                        </div>
                        <div data-testid="polymarket-placed-identity" className="rounded border border-[var(--border-subtle)] bg-[var(--surface-panel)] px-3 py-2">
                          <div className="font-semibold text-[var(--text-primary)]">Polymarket placed leg</div>
                          <div className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1"><span className="text-[var(--text-secondary)]">Selected market choice</span><span className={hasCanonicalOutcomes ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}>{hasCanonicalOutcomes ? position.pmOutcomeLabel!.trim() : 'Unavailable'}</span><span className="text-[var(--text-secondary)]">Side</span><span className="font-semibold text-[var(--text-primary)]">{position.pmSide.toUpperCase()}</span><span className="text-[var(--text-secondary)]">Platform question</span><span className={position.pmMarketQuestion?.trim() ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}>{position.pmMarketQuestion?.trim() || 'Unavailable'}</span><span className="text-[var(--text-secondary)]">Condition ID</span><span className={`break-all font-mono ${position.pmConditionId ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}`}>{position.pmConditionId || 'Unavailable'}</span><span className="text-[var(--text-secondary)]">Held token</span><span className={`break-all font-mono ${position.pmEntryTokenId ? 'text-[var(--text-primary)]' : 'text-[var(--status-warning)]'}`}>{position.pmEntryTokenId || 'Unavailable'}</span></div>
                        </div>
                      </div>
