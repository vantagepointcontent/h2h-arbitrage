...
export function DecisionCommandCenter() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/execute", { cache: "no-store" }).then(response => response.json()),
    ]).then(([positionsResponse, executionsResponse, executionStatus]) => {
      if (!active) return;
      const positions = positionsResponse.positions ?? [];
      const credentialReady = {
        kalshi: executionStatus.credentials?.kalshi?.ready === true,
        polymarket: executionStatus.credentials?.polymarket?.ready === true,
      };
      const monitoringInactive = executionStatus.credentials?.allReady !== true;
      setSnapshot({
        queue: monitoringInactive ? [] : buildAttentionQueue(
          positions,
          executionsResponse.executions ?? [],
          positionsResponse.errors ?? {},
          { credentialReady },
        ),
        portfolio: summarizePortfolio(positions),
  ...
  return <div className="space-y-4">
    <section aria-label="Attention queue" className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-panel)] p-4">
      ...
      {snapshot.error ? <p className="text-xs text-[var(--status-negative)]">Risk context unavailable: {snapshot.error}</p> : snapshot.monitoringInactive ? <div className="rounded-lg border border-[var(--border-subtle)] bg-[var(--surface-muted)] px-3 py-2 text-xs text-[var(--text-secondary)]"><span className="font-semibold text-[var(--text-primary)]">Trading credentials not configured.</span> Position monitoring and execution risk detection are inactive.</div> : snapshot.queue.length === 0 ? <div className="flex items-center gap-2 text-xs text-[var(--text-secondary)]"><CheckCircle2 className="h-4 w-4 text-[var(--status-positive)]" /> No execution or position risk currently requires action.</div> : <div className="grid gap-2 lg:grid-cols-2">{snapshot.queue.slice(0, 8).map(item => <div key={item.id} className={`rounded-lg border px-3 py-2 ${item.severity === 3 ? "border-[var(--status-negative)]/30 bg-[var(--status-negative)]/5" : item.severity === 2 ? "border-[var(--status-warning)]/30 bg-[var(--status-warning)]/5" : "border-[var(--border-subtle)] bg-[var(--surface-muted)]"}`}><div className="flex items-center gap-2 text-xs font-semibold"><AlertTriangle className={`h-3.5 w-3.5 ${item.severity === 3 ? "text-[var(--status-negative)]" : item.severity === 2 ? "text-[var(--status-warning)]" : "text-[var(--text-secondary)]"}`} />{item.title}</div><p className="mt-1 truncate text-xs text-[var(--text-secondary)]">{item.detail}</p></div>)}</div>}
    </section>
