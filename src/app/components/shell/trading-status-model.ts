...
export function buildTradingStatus(input: TradingStatusInput): RailItem[] {
  ...
  return [
    ...
    { id: "risk", label: "Portfolio risk", value: unhedged.length ? `${unhedged.length} unhedged · $${exposure.toFixed(2)}` : partial ? `${partial} partial fill${partial === 1 ? "" : "s"}` : `${positions} open · Hedged`, tone: unhedged.length ? "critical" : partial || positionErrors.length ? "warning" : "neutral", detail: `${positions} open positions. ${unhedged.length} unhedged executions. ${partial} partial fills.${positionErrors.length ? ` Position sources degraded: ${positionErrors.join("; ")}` : ""}`, remediation: unhedged.length || partial ? "Inspect Trades and close or hedge affected legs manually." : positionErrors.length ? "Restore authenticated position feeds." : undefined },
  ];
}
