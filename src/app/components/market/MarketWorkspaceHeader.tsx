...
import type { VenuePriceFreshness, VenuePriceFreshnessMap } from "@/app/lib/page-shared";
import { formatSavedMarketLifecycleSummary, type SavedMarketLifecycle } from "@/lib/saved-market-lifecycle";
...
interface Props {
  ...
  priceFreshness: VenuePriceFreshnessMap;
  nowMs?: number;
  loading: boolean;
  refreshing: boolean;
  favorite: boolean;
...
const PRICE_STALE_AFTER_MS = 15 * 60_000;

const statusLabels: Record<VenuePriceFreshness['status'], string> = {
  fresh: 'Fresh',
...
function formatAge(observedAt: string | null, now: number): string | null {
  const observedMs = observedAt ? Date.parse(observedAt) : NaN;
  if (!Number.isFinite(observedMs)) return null;
  const seconds = Math.max(0, Math.round((now - observedMs) / 1_000));
...
function priceAgeMetric(platform: 'Kalshi' | 'Polymarket', freshness: VenuePriceFreshness, now: number) {
  const observedMs = freshness.observedAt ? Date.parse(freshness.observedAt) : NaN;
  const effectiveStatus = freshness.status === 'fresh' && Number.isFinite(observedMs)
    && now - observedMs > PRICE_STALE_AFTER_MS ? 'stale_last_known' : freshness.status;
  const status = statusLabels[effectiveStatus];
  const age = formatAge(freshness.observedAt, now);
  const value = age ? `${status} · ${age}` : `Age unavailable · ${status}`;
  const timestamp = freshness.observedAt ?? 'No trustworthy timestamp';
  const source = freshness.source ?? 'No persisted snapshot source';
  const reason = freshness.reason ?? 'No failure reported';
  return {
    label: platform === 'Polymarket' ? 'PM price age' : 'Kalshi price age',
    value,
    description: `${platform} price age: ${value}. Timestamp: ${timestamp}. Source: ${source}. Reason: ${reason}.`,
  };
}
...
export function MarketWorkspaceHeader(props: Props) {
  ...
  const clock = props.nowMs ?? now;
  const missingFreshness: VenuePriceFreshness = {
    status: 'not_scanned',
    observedAt: null,
    source: null,
    reason: 'No trustworthy per-venue freshness was persisted for this cached market result',
  };
  const priceAgeMetrics = [
    priceAgeMetric('Kalshi', props.priceFreshness?.kalshi ?? missingFreshness, clock),
    priceAgeMetric('Polymarket', props.priceFreshness?.polymarket ?? missingFreshness, clock),
  ];
  const lifecycleSummary = props.lifecycle ? formatSavedMarketLifecycleSummary(props.lifecycle, clock) : null;
  ...
  return <section aria-label="Active market workspace" className="rounded-[var(--radius-panel)] border border-[var(--border-subtle)] bg-[var(--surface-panel)]">
    <div className="flex flex-col gap-3 p-3 lg:flex-row lg:items-center lg:p-4">
      ...
      <div className={`${expanded ? "grid" : "hidden"} grid-cols-2 gap-1.5 lg:grid lg:grid-cols-5`}>
        {[['Best net ROI', metrics.bestNetRoi == null ? '—' : `${metrics.bestNetRoi.toFixed(2)}%`],['Best net profit',usd(metrics.bestNetProfit)],['Max executable',usd(metrics.maxExecutableStake)]].map(([label,value]) => <div key={label} title={metricTooltips[label]} className="min-w-[112px] rounded-[var(--radius-control)] border border-[var(--border-subtle)] bg-[var(--surface-raised)] px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">{label}</div><div className="mt-0.5 text-xs font-bold tabular-nums text-[var(--text-primary)]">{value}</div></div>)}
        {priceAgeMetrics.map(({ label, value, description }) => <div key={label} title={description} aria-label={description} className="min-w-[132px] rounded-[var(--radius-control)] border border-[var(--border-subtle)] bg-[var(--surface-raised)] px-2.5 py-1.5"><div className="text-[10px] uppercase tracking-wide text-[var(--text-muted)]">{label}</div><div className="mt-0.5 whitespace-nowrap text-xs font-bold tabular-nums text-[var(--text-primary)]">{value}</div></div>)}
      </div>
    ...
    {lifecycleSummary && props.lifecycle?.overallStatus !== 'fresh' && <div role="status" title={lifecycleSummary.reason ?? undefined} className="border-t border-[var(--border-subtle)] px-4 py-2 text-[10px] text-[var(--status-warning)]">{lifecycleSummary.label}</div>}
