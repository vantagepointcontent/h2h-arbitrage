...
async function writeLiveResult(
  ...
  const liveResult: LastScanResult = {
    ...
    allArbs: clean.map((r) => ({
      artist: r.artist,
      kalshiMarketQuestion: r.kalshiMarketQuestion ?? null,
      pmMarketQuestion: r.pmMarketQuestion ?? null,
      kalshiOutcomeLabel: r.kalshiOutcomeLabel ?? null,
      pmOutcomeLabel: r.pmOutcomeLabel ?? null,
      relationshipVerified: r.relationshipState === 'verified_complementary',
      relationshipState: r.relationshipState,
      relationshipExplanation: r.relationshipExplanation ?? null,
      ...
      const persisted = await persistAndConsumeBotScan(pairId, {
        ...
        raw: {
          ...
          allArbs: clean.map((r) => ({
            ...
            pmSide: r.pmSide,
            strategy: r.strategy,
            arbType: r.arbType ?? undefined,
            propositionRelationship: r.propositionRelationship ?? null,
            roiPct: r.roiPct,
            expectedProfit: r.expectedProfit,
            apyPct: canonicalApyFor(r).apyPct,
            daysToExpiry: canonicalApyFor(r).daysToExpiry,
            ...
            pmYesTokenId: r.pmYesTokenId,
            pmNoTokenId: r.pmNoTokenId,
            ...
            kalshiYesExecutableQuote: r.kalshiYesExecutableQuote,
            kalshiNoExecutableQuote: r.kalshiNoExecutableQuote,
            pmYesExecutableQuote: r.pmYesExecutableQuote,
            pmNoExecutableQuote: r.pmNoExecutableQuote,
            pmYesMinOrderSize: r.pmYesMinOrderSize ?? null,
            pmNoMinOrderSize: r.pmNoMinOrderSize ?? null,
            pmYesTickSize: r.pmYesTickSize ?? null,
            pmNoTickSize: r.pmNoTickSize ?? null,
            executionStatus: r.executionStatus,
            executionBlocker: r.executionBlocker,
            fees: r.fees,
            stale: r.stale,
          })),
        },
